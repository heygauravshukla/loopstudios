...
const Header = () => {
    ...
    return (
        <header className={`${isNavExpanded ? "fixed inset-x-5" : "static"} flex justify-between items-center z-30`}>
            <Link href="/" className='z-30'>
                <Image className='max-h-[1.5rem] w-auto' src="/logo.svg" alt="Logo Image" height={32} width={192} />
            </Link>
            <button onClick={toggleMenu} className={`${isNavExpanded ? "bg-[url('/icon-close.svg')]" : "bg-[url('/icon-hamburger.svg')]"} w-[1.5rem] h-[1.5rem] bg-no-repeat bg-contain bg-center z-30`}></button>
